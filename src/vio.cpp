...
// 通过最小化当前图像与参考图像的光度误差来优化相机位姿，更新EKF状态
void VIOManager::updateState(cv::Mat img, int level)
{
  if (total_points == 0) return; // 如果没有视觉特征点，则直接返回
  StatesGroup old_state = (*state); // 保存当前状态以备后续使用

  VectorXd z; // 测量残差向量
  MatrixXd H_sub; // 雅可比矩阵
  bool EKF_end = false; // 标志位，指示EKF更新是否结束
  float last_error = std::numeric_limits<float>::max(); // 上一次迭代误差

  const int H_DIM = total_points * patch_size_total; // 观测维度
  z.resize(H_DIM); // 调整残差向量大小
  z.setZero(); 
  H_sub.resize(H_DIM, 7); // 雅可比矩阵(6DOF位姿 + 1个曝光时间)
  H_sub.setZero();
